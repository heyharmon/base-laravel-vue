		...
		<div class="w-full max-w-md rounded-lg border border-neutral-200 bg-white p-8 shadow-sm">
			...
			<form v-if="!success" @submit.prevent="requestReset" class="space-y-4">
				...
				<div class="text-center text-sm text-neutral-600">
					<router-link to="/login" class="font-medium text-neutral-900 hover:underline"> Back to Login </router-link>
				</div>
			</form>

			<div v-if="success" class="text-sm">
				<button @click="router.push('/login')" class="font-medium text-neutral-900 hover:underline">Back to Login</button>
			</div>
		</div>
	</div>
</template>
