...
const resetPassword = async () => {
	...
	try {
		...
		error.value = err.email || 'An error occurred. Please try again.'
	} finally {
		loading.value = false
	}
}
...
<template>
	<div class="flex min-h-screen items-center justify-center bg-neutral-50 px-4">
		<div class="w-full max-w-md rounded-lg border border-neutral-200 bg-white p-8 shadow-sm">
			<h1 class="mb-6 text-2xl font-bold text-neutral-900">Set New Password</h1>
			...
			<form @submit.prevent="resetPassword" class="space-y-4">
				...
				<div>
					...
					<input
						...
						required
						readonly
						class="w-full rounded-md border border-neutral-300 bg-neutral-100 px-3 py-2 text-neutral-900 focus:border-neutral-500 focus:outline-none"
					/>
				</div>

				<div>
					<label for="password" class="mb-1 block text-sm font-medium text-neutral-700">New Password</label>
					<input
						id="password"
						v-model="password"
						type="password"
