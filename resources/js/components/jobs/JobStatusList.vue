...
<template>
	<div>
		<div>
			<!-- No jobs -->
			<div v-if="jobs.length === 0" class="py-4 text-center text-neutral-500">No jobs found.</div>

			<!-- Job list grouped by batch -->
			<div v-else>
				<!-- Batched Jobs -->
				<div v-if="processedBatches.length > 0" class="space-y-4 mb-4">
					<div v-for="batch in processedBatches" :key="batch.id" class="border-2 border-neutral-300 rounded-lg overflow-hidden bg-neutral-50">
						<div class="px-4 py-3 flex items-center justify-between bg-neutral-100 border-b">
							<div>
								...
								<span class="ml-2 text-sm text-neutral-500">{{ moment(batch.created_at).format('M/D/YYYY, h:mm:ss A') }}</span>
							</div>
						...
						<div class="divide-y divide-neutral-200">
							<div v-for="job in batch.jobs" :key="job.job_id" class="px-4 py-3">
								<div class="flex items-center justify-between mb-2">
									...
									<div class="flex items-center gap-2 text-xs text-neutral-500">
										...
										<span v-else>{{ moment(job.created_at).format('h:mm A') }}</span>
									</div>
								</div>
								...
								<div v-if="job.status === 'processing'">
									<div class="h-1.5 bg-neutral-200 rounded-full overflow-hidden">
										<div class="h-1.5 bg-blue-500 transition-all duration-500" :style="{ width: job.progress + '%' }"></div>
									</div>
				...
				<div v-if="nonBatchedJobs.length > 0" class="space-y-3">
					<div v-if="processedBatches.length > 0" class="text-sm font-medium text-neutral-500 mt-4 mb-2">Individual Runs</div>
					<div v-for="job in nonBatchedJobs" :key="job.job_id" class="border rounded-lg overflow-hidden bg-neutral-50">
						<div class="px-4 py-3 flex items-center justify-between border-b">
							<div>
								<span class="font-medium">{{ getJobName(job) }}</span>
								<span class="ml-2 px-2 py-1 text-xs rounded-full" :class="getStatusClass(job.status)">
									{{ job.status }}
								</span>
							</div>
							<div class="text-sm text-neutral-500">
								{{ moment(job.created_at).format('M/D/YYYY, h:mm:ss A') }}
							</div>
						</div>
						...
						<div class="px-4 py-3">
							...
							<div v-if="job.status === 'pending' || job.status === 'processing'" class="mb-3">
								<div class="h-2 bg-neutral-200 rounded-full overflow-hidden">
									<div class="h-2 bg-blue-500 transition-all duration-500" :style="{ width: job.progress + '%' }"></div>
								</div>
								<div class="mt-1 text-xs text-right">{{ job.progress }}%</div>
							</div>
